...
,tbl_install as (
  ...
  from `gcp-bi-elephant-db-gold.applaydu.user_activity` where 10=10 and date(active_date) >= '2025-01-01' and date(active_date) < date_add('2025-01-31', INTERVAL 1 DAY) 
    and install_country IN (select country from `applaydu.tbl_country_filter` where 2=2  )
    and install_source IN (select install_source from `gcp-bi-elephant-db-gold.applaydu.user_activity` where 10=10 and date(active_date) >= '2025-01-01' and date(active_date) < date_add('2025-01-31', INTERVAL 1 DAY) )
...
tbl_launch_resume_src as (
    select 'All' as period,
            ...
            SAFE_DIVIDE(SUM(time_spent), COUNT(DISTINCT user_id)) as time_result,
            ...
            SAFE_DIVIDE(SUM(time_spent), SUM(CASE WHEN (session_id=1 OR CAST(time_between_sessions AS INT)>=30) THEN 1 ELSE 0 END)) as time_result_sessions,
            --FORMAT_TIMESTAMP('%M min %S sec', TIMESTAMP_SECONDS(time_result_sessions)) AS `Average Time per Session`,
            SAFE_DIVIDE(SUM(CASE WHEN (session_id=1 OR CAST(time_between_sessions AS INT)>=30) THEN 1 ELSE 0 END), COUNT(DISTINCT user_id)) as `Average Session per User`,
            ...
            SAFE_DIVIDE(SUM(CASE WHEN install_date>=(select min(server_date) from `applaydu.tbl_date_filter` where 2=2 ) 
                    and install_date<date_add((select max(server_date) from `applaydu.tbl_date_filter` where 2=2 ), INTERVAL 1 DAY) 
            ...
            SAFE_DIVIDE(SUM(CASE WHEN ((session_id=1 OR CAST(time_between_sessions AS INT)>=30) 
                         and install_date>=(select min(server_date) from `applaydu.tbl_date_filter` where 2=2 ) 
                         and install_date<date_add((select max(server_date) from `applaydu.tbl_date_filter` where 2=2 ), INTERVAL 1 DAY)) 
...
tbl_users_src as (
    select 'All' as period,
            ...
            SAFE_DIVIDE(SUM(CASE WHEN scans>0 THEN total_time_spent ELSE 0 END), COUNT(DISTINCT CASE WHEN scans>0 THEN user_id ELSE 0 END)) as time_result_scan_users,
            ...
            SAFE_DIVIDE(SUM(CASE WHEN scans>0 THEN total_time_spent ELSE 0 END), SUM(CASE WHEN scans>0 THEN sessions_count ELSE 0 END)) as time_result_session_scan_users,
            ...
            SAFE_DIVIDE(SUM(CASE WHEN scans>0 and install_date>=(select min(server_date) from `applaydu.tbl_date_filter` where 2=2 ) 
                    and install_date<date_add((select max(server_date) from `applaydu.tbl_date_filter` where 2=2 ), INTERVAL 1 DAY) 
            ...
            SAFE_DIVIDE(SUM(CASE WHEN scans>0 and install_date<(select min(server_date) from `applaydu.tbl_date_filter` where 2=2 ) 
                    THEN total_time_spent ELSE 0 END), SUM(CASE WHEN scans>0 and install_date<(select min(server_date) from `applaydu.tbl_date_filter` where 2=2 ) 
                    THEN sessions_count ELSE 0 END)) as time_result_session_scan_old_users,
            ...
            SAFE_DIVIDE(SUM(CASE WHEN scans=0 THEN total_time_spent ELSE 0 END), COUNT(DISTINCT CASE WHEN scans=0 THEN user_id ELSE 0 END)) as time_result_free_users,
            ...
            SAFE_DIVIDE(SUM(CASE WHEN scans=0 THEN total_time_spent ELSE 0 END), SUM(CASE WHEN scans=0 THEN sessions_count ELSE 0 END)) as time_result_session_free_users,
            ...
            SAFE_DIVIDE(SUM(CASE WHEN scans=0 and install_date>=(select min(server_date) from `applaydu.tbl_date_filter` where 2=2 ) 
                    ...
                    THEN total_time_spent ELSE 0 END), SUM(CASE WHEN scans=0 and install_date>=(select min(server_date) from `applaydu.tbl_date_filter` where 2=2 ) 
                    and install_date<date_add((select max(server_date) from `applaydu.tbl_date_filter` where 2=2 ), INTERVAL 1 DAY) 
                    THEN sessions_count ELSE 0 END)) as time_result_session_free_new_users,
            ...
            SAFE_DIVIDE(SUM(CASE WHEN scans=0 and install_date<(select min(server_date) from `applaydu.tbl_date_filter` where 2=2 ) 
                    THEN total_time_spent ELSE 0 END), SUM(CASE WHEN scans=0 and install_date<(select min(server_date) from `applaydu.tbl_date_filter` where 2=2 ) 
                    THEN sessions_count ELSE 0 END)) as time_result_session_free_old_users
...
select 319 as dashboard_id
		,4250 as query_id
		,timestamp('2025-01-01') as start_date
		,timestamp('2025-01-31') as end_date
