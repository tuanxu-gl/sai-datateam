...
where start_date =  GREATEST((SELECT MIN(SERVER_DATE) FROM applaydu_not_certified.tbl_date_filter WHERE 1=1 [[AND {{idate}}]]), '2020-08-10')
    ...
    and ((select count(0) from tbl_country_filter ) = (select count(0) from tbl_country_filter where 1=1  [[AND {{icountry}}]]))
...
where  1=1
    ...
    and server_time >= (SELECT min(SERVER_DATE) from tbl_date_filter where 1=1 [[AND {{idate}}]] ) and server_time < dateadd(day, 1,(SELECT max(SERVER_DATE) from tbl_date_filter where 1=1 [[AND {{idate}}]] ))
