...
,tbl_install as (
  ...
  from `gcp-bi-elephant-db-gold.applaydu.user_activity` where 10=10 and date(active_date) >= 'istart_date' and date(active_date) < date_sub('iend_date', INTERVAL 3 DAY) 
    and install_country IN (select country from `applaydu.tbl_country_filter` where 2=2  )
    and install_source IN (select install_source from `gcp-bi-elephant-db-gold.applaydu.user_activity` where 10=10 and date(active_date) >= 'istart_date' and date(active_date) < date_sub('iend_date', INTERVAL 3 DAY) )
