...
where 1=1 and date(client_time) >= '2025-01-01' and date(client_time) < date_add('2025-01-31', INTERVAL 1 DAY)
...
where 1=1 and date(client_time) >= '2025-01-01' and date(client_time) < date_add('2025-01-31', INTERVAL 1 DAY) and time_spent>=0 and from_scene<>'Inkmagination' -- when users from AH Drawing to AH,we do not count as a new session
...
select time_spent
from gcp-bi-elephant-db-gold.applaydu.VISIT_SCREEN
where 1=1 and date(client_time) >= '2025-01-01' and date(client_time) < date_add('2025-01-31', INTERVAL 1 DAY) and time_spent>=0 and time_spent<36000
...
select time_spent
from gcp-bi-elephant-db-gold.applaydu.AVATAR_HOUSE_FINISHED
where 1=1 and date(client_time) >= '2025-01-01' and date(client_time) < date_add('2025-01-31', INTERVAL 1 DAY) and time_spent>=0
 and (version>='4.5.0' and date(client_time)>='2024-02-05') and version<'9.0.0' 
 and version>=(select min(version) from `applaydu.tbl_version_filter` where 2=2 ) and version<=(select max(version) from `applaydu.tbl_version_filter` where 2=2 )
 and version in (select version from `applaydu.tbl_version_filter` where 2=2 )
 and date(client_time)>=(select min(server_date) from `applaydu.tbl_date_filter` where 2=2 ) and date(client_time)<date_add((select max(server_date) from `applaydu.tbl_date_filter` where 2=2 ), INTERVAL 1 DAY)
...
from dedicated 
 join (select distinct user_id from `gcp-bi-elephant-db-gold.applaydu.user_activity` where 10=10 and date(active_date) >= '2025-01-01' and date(active_date) < date_add('2025-01-31', INTERVAL 1 DAY) ) using (user_id)
union all
