DECLARE r14_q288 ARRAY<STRUCT<`Week` STRING,`New Installations` INT64>>;
  ...
  IF row_count = 0 THEN
    SET r14_q288 = (
      SELECT ARRAY(
        --main query
SELECT AS STRUCT 
    cast(DATE_TRUNC(DATE(client_time), WEEK) as string) AS `Week`,
...
GROUP BY 
  ...
  ELSE
    SET r14_q288 = (
      SELECT ARRAY_AGG(
        STRUCT(
           CAST(value1_str as STRING) as `Week`, CAST(value2 as INT64) as `New Installations`
        )
      )
      FROM 
