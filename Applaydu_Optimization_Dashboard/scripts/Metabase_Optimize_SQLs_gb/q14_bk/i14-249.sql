DECLARE r14_q249 ARRAY<STRUCT<`Shop` STRING,`Total Installations` INT64>>;
  ...
  IF row_count = 0 THEN
    SET r14_q249 = (
      SELECT ARRAY(
        --main query
SELECT AS STRUCT REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(CAST(t.game_id AS STRING), '81335', 'App Store'), '81337', 'Google Play'), '82471', 'AppInChina'), '85247', 'AppInChina'), '84515', 'Samsung'), '85837', 'Amazon') AS `Shop`, 
  ...
  ELSE
    SET r14_q249 = (
      SELECT ARRAY_AGG(
        STRUCT(
           CAST(value1_str as STRING) as `Shop`, CAST(value2 as INT64) as `Total Installations`
        )
      )
      FROM 
