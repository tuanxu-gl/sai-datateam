DECLARE r319_q4256 ARRAY<STRUCT<users INT64,sum_total_time_spent INT64,time_result FLOAT64,`Time spent` STRING>>;
...
WHERE 
    ...
    AND scan_mode_finished_count = 0
    )
  );
...
ELSE
  SET r319_q4256 = (
    SELECT ARRAY_AGG(
      STRUCT(
         CAST(value1 as INT64) as users, CAST(value2 as INT64) as sum_total_time_spent, CAST(value3 as FLOAT64) as time_result, CAST(value4_str as STRING) as `Time spent`
      )
    )
    FROM 
