DECLARE r319_q4246 ARRAY<STRUCT<`Total Scan Users` INT64>>;
...
--main query
SELECT AS STRUCT COUNT(DISTINCT user_id) AS `Total Scan Users`
FROM (
    SELECT * FROM tbl_utm 
    UNION ALL 
...
ELSE
  SET r319_q4246 = (
    SELECT ARRAY_AGG(
      STRUCT(
         CAST(value1 as INT64) as `Total Scan Users`
      )
    )
    FROM 
