DECLARE r319_q4272 ARRAY<STRUCT<`Total Scans Users` INT64>>;
...
IF row_count = 0 THEN
  SET r319_q4272 = (
    SELECT ARRAY(
      --main query
SELECT AS STRUCT COUNT(DISTINCT user_id) AS `Total Scans Users`
...
ELSE
  SET r319_q4272 = (
    SELECT ARRAY_AGG(
      STRUCT(
         CAST(value1 as INT64) as `Total Scans Users`
      )
    )
    FROM 
