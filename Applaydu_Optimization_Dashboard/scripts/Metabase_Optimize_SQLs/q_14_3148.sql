with r3148 as(
SELECT value1_str as environment,value2 as "Users replay",value3 as "Total users",value4 as percentage
...
where start_date =  GREATEST((SELECT MIN(SERVER_DATE) FROM applaydu_not_certified.tbl_date_filter WHERE 1=1 [[AND {{idate}}]]), '2020-08-10')
	...
	and dashboard_id=14 and query_id = 3148 
)
...
where 1=1 and (select available from tbl_check_preprocess_report) = 'N/A' and environment_id = 'Eduland Lets Story' and from_scene = 'Eduland Lets Story'
  ...
  and client_time >= (select ivalue from APPLAYDU_NOT_CERTIFIED.TBL_VARIABLES where ikey = 'apd_v5_lets_story_start_date')  and client_time < current_date()
  and client_time < dateadd(day, 1,(SELECT max(SERVER_DATE) from tbl_date_filter where 2=2 [[AND {{idate}}]] ))
